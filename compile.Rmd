...
country_year_final <- country_year %>%
  ...
  #mutate(devel = pec >= median(pec, na.rm = TRUE)) %>%
  select(stateabb, ccode, year, cinc, pec, majpow, regimetype, autoc) %>%
  left_join(diffs)
...
second_join <- first_join %>% left_join(country_year_final %>% select(-lapse), by = c("ccode2" = "ccode", "year"))
...
almost_done <- third_join %>%
  mutate(
    ...
    #autoc = ifelse(homogenous, autoc.x, "mixed"),
...
almost_done %>% filter(is.na(lapse), !mid)
```
