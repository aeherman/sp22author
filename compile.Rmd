...
first_join <- expanded %>% filter(year >= 1945, year < 2011) %>%
...
almost_done <- third_join %>%
  ...
  left_join(year_ally) %>%
  mutate(across(c(allied, cont), ~ifelse(is.na(.), FALSE, .)),
         pec_bi_lag = c(0, almost_done$pec_bi[-nrow(almost_done)])) %>%
  filter(year > 1945) %>%
  distinct %>%
  select(ccode1, ccode2, year, mid, dispnum, homogenous,
         dyadtype, autoc, cont, allied, majpow, relpow, pec_bi, pec_bi_lag, log_pec, lapse)
